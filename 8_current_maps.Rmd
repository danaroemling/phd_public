...
merger <- merger %>% filter(TokenCount > 1456)

# alternations
one_word <- corpus %>% filter(word == "abitur")
colnames(one_word) <- c("Token", "City", "Frequency")
second_word <- corpus %>% filter(word == "matura")
...
ggplot() +
  ...
  theme_minimal() +
  scale_fill_continuous(low="#ffffff", high="#310d59", name = "'nicht'", labels = NULL) +
...
```{r dark mode plot N bands red}
# Change the number of n.breaks = to adjust number of break points
ggplot() +
  geom_sf(data = krig2, aes(fill = var1.pred), shape = 21, size = 0.75, stroke = 0, lwd = 0) +
  geom_sf(data = gsa_plot, aes(geometry = geometry), color = "snow2", fill = NA, size = 0.5) +
  geom_sf_text(data = cities_sf, aes(label = City), size = 2.5, nudge_x = 0, nudge_y = -0.15, color = "snow", family = "Optima") +
  geom_sf(data = cities_sf, aes(geometry = geometry), shape = 4, color = "snow") +
  theme_minimal() +
  scale_fill_steps(low="#000000", high="firebrick1", n.breaks = 30, labels = NULL) +
  theme(axis.title.x = element_blank(), 
        axis.title.y = element_blank(),
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
...
```{r dark mode plot comparison red blue}
ggplot() +
    geom_sf(data = krig_comp_2, aes(fill=var1.pred), shape = 21, size = 0.5, stroke = 0, lwd = 0) +
    geom_sf(data = gsa_plot, aes(geometry = geometry), color = "snow2", fill = NA, size = 0.5) +
    geom_sf_text(data = cities_sf, aes(label = City), size = 2.5, nudge_x = 0, nudge_y = -0.15, family = "Optima", color="snow2") +
    ...
    theme(axis.title.x = element_blank(), 
          ...
          panel.grid.major = element_blank()) +
    theme(legend.position = c(0.90, 0.65), 
          legend.title = element_text(size = 6, family = "Optima", color = "snow"),
          legend.text = element_text(size = 6, family = "Optima", hjust = 1, color = "snow"),
          legend.title.align = 0,
          legend.key.size = unit(0.3, "cm"),
          legend.key.width = unit(0.4,"cm"),
          panel.background = element_rect(fill = "#303030", color = "#303030"),
          plot.background = element_rect(fill = "#303030", color = "#303030"),
          plot.title = element_text(hjust = 0.5, color = "snow", family = "Optima")) +
...
ggplot() +
  ...
  scale_fill_steps2(low = "seagreen4", mid = "#ffffff", high = "mediumpurple4", midpoint = 0.5, n.breaks = 15, labels = NULL) +
    theme(axis.title.x = element_blank(), 
          axis.title.y = element_blank(),
          axis.text.x = element_blank(),
          axis.text.y = element_blank(),
